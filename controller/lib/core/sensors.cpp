...
void TVIntegrator::AddFlow(Time now, VolumetricFlow flow) {
  ...
  // I made a point to keep the VOLUME_INTEGRAL_INTERVAL to drive volume
  // updates, as I think it is a good solution to filter HF noise, though I am
  // not sure this is actually necessary. This could be made much clearer if the
  // volume is updated on every loop instead of every VOLUME_INTEGRAL_INTERVAL
